...
Recently, I was working on a implementation where I needed to look at how the existing Java code was written. I used a tool called `JD-GUI` to change the code back to decompile the code. When I was trying to save the decompiled class `JD-GUI` added extra comments that I didn't need. It was easy to delete these comments for small parts of the code, but for large java class or on batch selection, it became really hard. So, I decided to make my own tool that decompiles Java code into .java form without unnecessary comments.
